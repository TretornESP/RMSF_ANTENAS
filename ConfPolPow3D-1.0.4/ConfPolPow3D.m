...
function ArrBrowse_button_Callback(hObject, eventdata, handles, solving)
...
if exist('solving','var')
    handles = guidata(hObject);
    fullFileName = handles.InFile;
    [fol, baseFileNameNoExt, extension] = fileparts(fullFileName);
    handles.OutFile = fullfile(fol,baseFileNameNoExt);
else
    [baseName, folder] = uigetfile('../*.dat');
    fullFileName = fullfile(folder, baseName);
    handles.OutFile = folder;
end;
...
[X,Y,Z,Amp,PhaDeg,AXn,BXn,GXn,AYn,BYn,GYn,KElem,KModif]=textread(fullFileName,'%f %f %f %f %f %f %f %f %f %f %f %d %d');
...
if(handles.ConfStored == 0),     % If the configuration is not ready, notify the user
    ...
    Amp = handles.Amp; PhaRad = handles.PhaRad;  KElem = handles.KElem; KModif = handles.KModif;             % Excitations and kind of element
   ...
   % Now build up the inverse component matrices
    ...
    % Show a progress bar
    WB = waitbar(0,'Calculating 3D fields... please wait...','visible',handles.Solver);
    ...
    for n = 1:NElem; % Take a loop on each element
        ...
        AmpT =repmat(Amp(n),NPTh,NPPh); PhaRadT = repmat(PhaRad(n),NPTh,NPPh); K = repmat(KElem(n),NPTh,NPPh); M = repmat(KModif(n),NPTh,NPPh);
        ...
        % in matrix form
        [Felx Fely Felz] = arrayfun(@PolElemF,Thetan,Phin,Xnx,Xny,Xnz,Ynx,Yny,Ynz,Znx,Zny,Znz,K, M);
    ...
    if(get(handles.Phi_radb,'Value') == 1), %If the user has selected a Phi = constant cut
        ...
        WB = waitbar(0,'Calculating pattern cuts... please wait...','visible',handles.Solver);
        ...
        for n = 1:NElem;  % Take a loop concerning each element
            ...
            % And the same for the excitations, kind of fields and positions of the elements
            AmpT =repmat(Amp(n),NPPlanar,1); PhaRadT = repmat(PhaRad(n),NPPlanar,1); K = repmat(KElem(n),NPPlanar,1); M = repmat(KModif(n),NPPlanar,1);
        ...
        for n = 1:NElem;   % Take a loop concerning each element
            ...
            % And the same for the excitations, kind of fields and positions of the elements
            AmpT =repmat(Amp(n),1,NPPlanar); PhaRadT = repmat(PhaRad(n),1,NPPlanar); K = repmat(KElem(n),1,NPPlanar); M = repmat(KModif(n),1,NPPlanar);
...
if (handles.PlotAmp == 1), % If the amplitude plot is visible
    ...
    f = figure('Name','Excitation Amplitude Distribution','NumberTitle','off','visible', handles.Solver);     % Create a new figure
    AmpFact = str2num(get(handles.ConesMaxLength,'String'));                    % Get amplitude factor 
    RadFact = str2num(get(handles.CylRadius,'String'));                         % Get radius factor
    ...
    saveas(f,fullfile(handles.OutFile, '2d_amplitude_distr.png'));
else
    disp("plotamp is 0");
end;
...
if (handles.PlotPha == 1), % If the phase plot is visible
    handles = guidata(hObject);                                                 % Get handler
    f = figure('Name','Excitation Phase Distribution','NumberTitle','off','visible',handles.Solver);         % Create a new figure
    ...
    saveas(f, fullfile(handles.OutFile, '2d_phase_distr.png'));
end;
...
if(handles.Plot3D == 1), % If the 3D power pattern is visible
    ...
    f = figure('Name','3D Polar Total Power Pattern','NumberTitle','off','visible',handles.Solver);              % Create a separate figure
    ...
    saveas(f, fullfile(handles.OutFile, strcat('3d_total_power_',get(handles.Angle_Value,'String'),'.png')));
    % Theta Component Pattern
    f = figure('Name','3D Polar Theta-component Power Pattern','NumberTitle','off','visible',handles.Solver);    % Create a separate figure
    ...
    alpha(Al);                                                                      % Set transparency
    saveas(f, fullfile(handles.OutFile, strcat('3d_theta_power_',get(handles.Angle_Value,'String'),'.png')));
    % Phi Component Pattern
    f = figure('Name','3D Polar Phi-component Power Pattern','NumberTitle','off','visible',handles.Solver);      % Create a separate figure
    ...
    alpha(Al);
    saveas(f, fullfile(handles.OutFile, strcat('3d_phi_power_',get(handles.Angle_Value,'String'),'.png')));
else
    disp("plot3d error");
end;
...
if (handles.PlotCut == 1), % If the cut is visible
    ...
    % Total Power Pattern
    f = figure('Name','2D Polar Total Power Pattern','NumberTitle','off','visible',handles.Solver);              % Create a separate figure
    ...
    axis off; 
    saveas(f, fullfile(handles.OutFile, strcat('2d_total_power_',get(handles.Angle_Value,'String'),'.png')));
        % Do the same with the other field components
        % Theta component
        f = figure('Name','2D Polar Theta Component Power Pattern','NumberTitle','off','visible',handles.Solver);
        PlanarPlot(handles.AngPlanar,handles.RThPlanar,LeveldB,TtlTh,TxtSz,PopUp,ThC); 
        axis off;
        saveas(f, fullfile(handles.OutFile, strcat('2d_theta_power_',get(handles.Angle_Value,'String'),'.png')));
        % Phi component
        f = figure('Name','2D Polar Phi Component Power Pattern','NumberTitle','off','visible',handles.Solver);
        PlanarPlot(handles.AngPlanar,handles.RPhPlanar,LeveldB,TtlPh,TxtSz,PopUp,ThC);
        axis off;
        saveas(f, fullfile(handles.OutFile, strcat('2d_phi_power_',get(handles.Angle_Value,'String'),'.png')));
else
    disp("Cut error");
end;
...
function [Felx Fely Felz] = PolElemF(Thetan,Phin,CosAnx,CosBnx,CosGnx,CosAny,CosBny,CosGny,CosAnz,CosBnz,CosGnz,KElem, KModif)
% Arguments:
...
% --- Executes on button press in SolveP1.
function SolveP1_Callback(hObject, eventdata, handles)
% hObject    handle to SolveP1 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

if (get(handles.AmplDist_chck,'Value')==0)
    [baseName, folder] = uigetfile('*.dat', 'MultiSelect', 'on');
else
    folder = './DATOS_PRACTICA/';
    resources = dir('./DATOS_PRACTICA/*.dat');
    for i = 1:size(resources, 1)
        baseName{i} = resources(i).name;
    end;
end;
if isstr(baseName), baseName={baseName}; end;
w = waitbar(0, 'Solving figures');
for i = 1:length(baseName)
    fullFileName = fullfile(folder, baseName{i});
    [fol, baseFileNameNoExt, extension] = fileparts(fullFileName);
    target_folder = fullfile(fol,baseFileNameNoExt);
    if ~exist(target_folder, 'dir')
        mkdir(target_folder);
    end;
    handles.InFile = fullFileName;
    guidata(hObject, handles);

    ArrBrowse_button_Callback(hObject, eventdata, handles, 1);

    handles = guidata(hObject);

    DrawArr_button_Callback(hObject, eventdata, handles);

    handles = guidata(hObject);

    PopupAmpl_button_Callback(hObject, eventdata, handles);
    PopupPha_button_Callback(hObject, eventdata, handles);

    Angle = 0;
    set(handles.Angle_Value,'String',num2str(Angle,'%10.2f'));
    set(handles.Angle_Slider,'Value',Angle);
    CalcPattern_button_Callback(hObject, eventdata, handles);
    DrawPatt_button_Callback(hObject, eventdata, handles);
    handles = guidata(hObject);

    Popup3D_button_Callback(hObject, eventdata, handles);
    PopupCut_button_Callback(hObject, eventdata, handles);
    ...
    Popup3D_button_Callback(hObject, eventdata, handles);
    PopupCut_button_Callback(hObject, eventdata, handles);
    waitbar((i/length(baseName)), w, sprintf('Figure (%10.2f/%10.2f)', i, length(baseName))); 
end;
close(w);
msgbox(sprintf('Graphs generated at %s', folder),'Work done!','help');;